...
I then cherry-pick all of these commits into a `merged_x.xx` branch with x.xx being the release version number.  Once I am satisfied with that this `merged_` branch has passed the automated tests, or failed for understandable reasons, I cherry-pick the commits into `Rav4-TSS2`, that I use as my daily driver branch.  The idea is that the `merged_x.xx` branch and `Rav4-TSS2` should remain consistent so that the results of the tests on `merged_` can be relied on.

Sometimed I may create a `Rav4-TSS2-____` branch if I want to test changes in the vehicle before commiting them to my daily driver branch.
